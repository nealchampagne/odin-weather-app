...
export const currentWeatherLoad = async (weatherData) => {
  ...
  icon.src = icons.find(
    (icon) => icon.path === `./${weatherData.currentConditions.icon}.svg`,
  ).file;
  ...
  const sunriseMoment = moment(
    ...
    .hours(parseInt(splitSunrise[0]))
    .minutes(parseInt(splitSunrise[1]))
    .seconds(0)
    .milliseconds(0);
  const sunsetMoment = moment(
    ...
    .tz(weatherData.timezone)
    .hours(parseInt(splitSunset[0]))
    .minutes(parseInt(splitSunset[1]))
    .seconds(0)
    .milliseconds(0);
  ...
  if (
    (0 <= windDir && windDir <= 11.25) ||
    (348.75 < windDir && windDir <= 360)
  ) {
    windCompDir = "N";
  ...
  for (let i = 1; i < 11; i++) {
    ...
    dailyIcon.src = icons.find(
      (icon) => icon.path === `./${weatherData.days[i].icon}.svg`,
    ).file;
